...
class TestSeriesSum(unittest.TestCase):
    ...
    sum() will be used to get the sum of the values for the requested axis.
    it will return the sum of the requested series
    paramerets:
    level:int or level name or default none
    if the axis is a multiindex(hierarchical),count along a perticular level,collapsing into a smaller series.
    returns
    sum of the values of requested axis.
    if you have an series[4,2,0,8]for names['dog','fish','cat','mouse']it will return
    dog    4
    fish   2 
    cat    0 
    mouse  8
